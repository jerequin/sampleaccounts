<?php
	...
	class CBXWPSimpleAccounting_Admin {
		...
		public function admin_menus() {
			...
			$sub_menu_account_hook = add_submenu_page( 'cbxwpsimpleaccounting', esc_html__( 'CBX Accounting: Manage Accounts', 'cbxwpsimpleaccounting' ), esc_html__( 'Manage Accounts', 'cbxwpsimpleaccounting' ), 'manage_options', 'cbxwpsimpleaccounting_accmanager', array(
				$this,
...
WHERE c.type = %d AND c.istaxincluded = %d AND MONTH(c.add_date) =%d AND YEAR(c.add_date) = %d ', 2, 1, $month, $year ) );
		...
		public function cbxwpsimpleaccounting_log_export() {
			...
			if ( isset( $_POST['_wpnonce'] ) && ! empty( $_POST['_wpnonce'] ) && isset( $_REQUEST['cbxwpsimpleaccounting_log_export'] ) && isset( $_REQUEST['format'] ) && $_REQUEST['format'] !== null ) {
				...
				if ( $export_format != 'pdf' ) {
					if ( ! class_exists( 'PHPExcel' ) ) {
						require_once CBXWPSIMPLEACCOUNTING_ROOT_PATH . '/includes/PHPExcel.php';
					}
				...
				if ( $pdf_lib_loaded || $phpexcel_lib_loaded ) {
					...
					if ( $pdf_lib_loaded ) {
						...
						$pdf->Cell( 0, 10, utf8_decode( __( 'Table View of Log Data', 'cbxwpsimpleaccounting' ) ), 0, 2, 'L', false );
						...
						$table_col_span = sizeof( $table_headings );
						...
						foreach ( $table_headings as $h ) {
							$table->easyCell( $h, 'font-color:#969696;font-style:B;' );
						}
						...
						/* $cbx_export_html = '<p>' . esc_html__('Type:', 'cbxwpsimpleaccounting') . $type . '</p>';
                    $cbx_export_html .= '<p>' . esc_html__('Category:', 'cbxwpsimpleaccounting') . $category_title . '</p>';
                    $cbx_export_html .= '<p>' . esc_html__('Account Name:', 'cbxwpsimpleaccounting') . $account_title . '</p>';
                    $cbx_export_html .= '<p>' . esc_html__('Date Range:', 'cbxwpsimpleaccounting') . $date_range . '</p>';*/

						//$cbx_export_html = apply_filters('cbxwpsimpleaccountinglog_export_pdf_before_table', $cbx_export_html);
						//$cbx_export_html = apply_filters('cbxwpsimpleaccountinglog_export_fpdf_before_table', $pdf);
					...
					if ( $datas ) {
						...
						foreach ( $datas as $i => $data ) {
							...
							if ( $pdf_lib_loaded ) {
								...
								$table_data[] = apply_filters( 'cbxwpsimpleaccountinglog_export_pdf_table_cols', array(
									'title'         => stripslashes( $data->title ),
									'amt'           => $data->amount,
									'type'          => $type,
									'category'      => $data->cattitle,
									'note'          => stripslashes( $data->note ),
									'added_by'      => stripslashes( get_user_by( 'id', $data->add_by )->display_name ),
									'mod_by'        => stripslashes( $mod_by ),
